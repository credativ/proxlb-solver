...
We define a 3D grid of boolean variables $x_{i,j,t} \in \{0, 1\}$:
*   $i \in \{1 \dots 	ext{Guests}\}$
*   $j \in \{1 \dots 	ext{Nodes}\}$
*   $t \in \{0 \dots T\}$ (where $T$ is the time horizon)

$x_{i,j,t} = 1$ means Guest $i$ resides on Node $j$ at step $t$.

### 3.2 State Transitions
Migration is defined as a change in position between $t$ and $t+1$. We introduce a transition variable $m_{i,t}$:
$$m_{i,t} \ge x_{i,j,t} - x_{i,j,t+1} \quad \forall j$$
...
$$\sum_{i} 	ext{Usage}_i \cdot x_{i,j,t} \le 	ext{Capacity}_j \quad \forall j, t$$
...
In 100% full clusters, a pure discrete model is too rigid. We introduce **Slack Variables** ($s_{j,t}$):
$$\sum_{i} 	ext{Usage}_i \cdot x_{i,j,t} \le 	ext{Capacity}_j + s_{j,t}$$
...
$$	ext{Penalty} = \sum 	ext{Violations} 	imes 	ext{RulePenalty}$$
...
$$	ext{Minimize: } W_B \cdot 	ext{LoadGap}_T + W_S \cdot \sum_{t} 	ext{Migrations}_t \cdot (t+1) + 	ext{SlackPenalty} + 	ext{RulePenalty}$$
...
Large $T$ values exponentially increase the search space ($Guests 	imes Nodes 	imes T$). 
The solver uses **Iterative Deepening**:
1.  Try $T=1$ (Direct moves).
2.  If infeasible or no improvement, try $T=2$ (Simple swaps).
3.  Expand to $T=4, 8, 16 \dots$ for complex cascades.
